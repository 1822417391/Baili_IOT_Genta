...
// Low Power defines
// 需要唤醒 37 次，每次 16.38 秒，总计约 606 秒
#define TOTAL_WAKEUPS 37
...
/********************************INIT Setting***********************************/
/* 低功耗功能变量声明 */
char					LOW_POWER_MODE;	// 低功耗模式标志：0-唤醒模式 1-断开太阳能发电池电量模式 2-低功耗模式
int 					SCCOUNTER;		// 低功耗模式SC计数器（休眠计时器）
int 					T0COUNTER;		// 低功耗模式T0计数器（T0定时器）
int 					RVCOUNTER;		// 低功耗模式RV计数器（读取电压计时器）
int 					LPCOUNTER;		// 低功耗模式LP计数器（低功耗计时器）
int						SCTIMEOUT = 10000;	//十分钟计时
int						WKCOUNTER;	// 低功耗模式WK计数器（唤醒计时器）
/********************************188 Setting***********************************/
...
void timer0_ISR() interrupt 1
{
    ...
    if(T0COUNTER > 50) {
		...
		//当休眠时间大于10分钟时，关闭太阳能充电
		if(SCCOUNTER > SCTIMEOUT){
			if(LOW_POWER_MODE == 0	|| LOW_POWER_MODE == 2){
				P32 = 0;
				P33 = 0;
				P37 = 0;
				LED = 1;
		...
		//当休眠时间大于10分钟时，关闭太阳能充电，并发完串口后，进入完全休眠
		if(SCCOUNTER > (SCTIMEOUT+50) && LOW_POWER_MODE == 1){
			SCCOUNTER = 0;
			TR0=0;
			WKTCH = 0xFF;
			WKTCL = 0xFE;
